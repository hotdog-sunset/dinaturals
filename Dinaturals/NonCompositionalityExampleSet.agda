...
{-
  We encode a combinatorial example of two dinaturals not composing, with Set as codomain.
  We use Bool and the constant function on true as a subcategory of Set which is equivalent to NonTrivialEndo.
-}
module Dinaturals.NonCompositionalityExampleSet where
...
idBool : Func BoolSetoid BoolSetoid
idBool = idS BoolSetoid

-- The constant function on true.
constTrue : Func BoolSetoid BoolSetoid
constTrue = record { to = λ _ → true ; cong = λ _ → refl }

OneSetoid : Setoid zeroℓ zeroℓ
OneSetoid = discreteSetoid ⊤

ZeroSetoid : Setoid zeroℓ zeroℓ
ZeroSetoid = discreteSetoid ⊥

F : Functor (Product (op Interval) Interval) (Setoids zeroℓ zeroℓ)
F = record
  { F₀ = λ { _ → BoolSetoid }
  ; F₁ = λ { _ → idS BoolSetoid }
  ; identity = refl
  ; homomorphism = refl
  ; F-resp-≈ = λ _ → refl
  }

G : Functor (Product (op Interval) Interval) (Setoids zeroℓ zeroℓ)
G = record
  { F₀ = λ { _ → BoolSetoid }
  ; F₁ = λ { {0F , 0F} {0F , 0F} (0F , 0F) → idS BoolSetoid
           ; {1F , 1F} {1F , 1F} (0F , 0F) → idS BoolSetoid
           ; {0F , 1F} {0F , 1F} (0F , 0F) → idS BoolSetoid
           ; {1F , 0F} {1F , 0F} (0F , 0F) → idS BoolSetoid

           ; {0F , 0F} {0F , 1F} (0F , 0F) → idS BoolSetoid
           ; {1F , 0F} {0F , 0F} (0F , 0F) → constTrue
           ; {1F , 1F} {0F , 1F} (0F , 0F) → idS BoolSetoid
           ; {1F , 0F} {1F , 1F} (0F , 0F) → constTrue

           ; {1F , 0F} {0F , 1F} (0F , 0F) → constTrue
...
H : Functor (Product (op Interval) Interval) (Setoids zeroℓ zeroℓ)
H = record
  { F₀ = λ { _ → BoolSetoid }
  ; F₁ = λ { {0F , 0F} {0F , 0F} (0F , 0F) → idS BoolSetoid
           ; {1F , 1F} {1F , 1F} (0F , 0F) → idS BoolSetoid
           ...
           ; {0F , 0F} {0F , 1F} (0F , 0F) → constTrue
           ; {1F , 1F} {0F , 1F} (0F , 0F) → idS BoolSetoid

           ; {1F , 0F} {0F , 0F} (0F , 0F) → idS BoolSetoid
           ; {1F , 0F} {0F , 1F} (0F , 0F) → constTrue
           ; {1F , 0F} {1F , 1F} (0F , 0F) → constTrue
...
do-not-compose comp = true≠false (comp {X = 0F} {Y = 1F} 0F {x = false})
